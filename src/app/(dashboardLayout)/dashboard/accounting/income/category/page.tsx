...
import IncomeCategoryModal from "./__components/IncomeCategoryModal"
import {
...
export default function IncomeCategoryPage() {
    ...
    const handleDelete = async (id: string) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await deleteIncomeCategory(id).unwrap()
                    if (res.success) {
    ...
    useEffect(() => {
        if (incomeCategories?.data?.data) {
            setCategories(incomeCategories.data.data)
        }
    }, [incomeCategories])
                        ...
                        <div className="mb-4 overflow-hidden bg-white shadow-sm rounded-3xl">
                                ...
                                <div className="grid grid-cols-12 gap-4 items-center">
                                    ...
                                    <div className="col-span-12 md:col-span-7 flex justify-end">
                                        <Box display="flex" alignItems="center" gap={1}>
                                            ...
                                            <Chip
                                                label={incomeCategories?.data?.meta?.total || 0}
                                                color="primary"
                                                size="small"
                                            />
                                        </Box>
                            ...
                            {isLoading ? (
                                ...
                                <>
                                    ...
                                    {/* Pagination */}
                                    {incomeCategories?.data?.meta?.total > rowsPerPage && (
                                        <TablePagination
                                            rowsPerPageOptions={[15, 25, 50]}
                                            component="div"
                                            count={incomeCategories?.data?.meta?.total || 0}
