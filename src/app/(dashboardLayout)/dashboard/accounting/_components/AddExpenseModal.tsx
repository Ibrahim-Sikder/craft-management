...
export default function AddExpenseModal({
  ...
  const { data: singleExpense, isLoading: singleExpenseLoading } =
  ...
  const [expenseItems, setExpenseItems] = useState<TExpenseItem[]>([
    { id: 1, source: "", amount: "" },
  ]);
  ...
  const handleExpneseItemChange = (
    id: number,
    field: keyof TExpenseItem,
    value: string,
  ) => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    const totalAmount = cleanedExpenseItems.reduce(
