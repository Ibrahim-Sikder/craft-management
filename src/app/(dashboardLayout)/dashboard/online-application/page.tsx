...
const StatusChip = ({ status }: { status: TAdmissionStatus }) => {
  const statusConfig: Record<
    TAdmissionStatus,
    { color: "success" | "warning" | "error"; icon: JSX.Element; label: string }
  > = {
    pending: {
      color: "warning",
...
export default function AdmissionApplications() {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          text:
            error?.data?.message ||
            `Failed to ${isApproval ? "approve" : "reject"} application`,
        });
      }
    }
  };
  ...
  const handleBulkAction = async (
    ...
    const title = isDelete
      ...
      : `Approve ${ids.length} Applications?`;
    const text = isDelete
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (failed === 0) {
          ...
          Swal.fire(
            "Partial Success",
            `${succeeded} ${isDelete ? "deleted" : "approved"}, ${failed} failed.`,
            "warning",
          );
        }
        ...
        Swal.fire(
          "Error",
          "Operation failed partially or completely.",
          "error",
        );
      }
    }
  ...
  const handleBulkApprove = (selectedRows: any[]) =>
    handleBulkAction(selectedRows, "approve");
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} md={9}>
              ...
              >
                <Chip
                  icon={<Pending />}
                  label={`Pending: ${data?.meta?.pending || 0}`}
                  color="warning"
                ...
                <Chip
                  icon={<CheckCircle />}
                  label={`Approved: ${data?.meta?.approved || 0}`}
                  color="success"
                  variant={
                    filters.status === "approved" ? "filled" : "outlined"
                  }
                ...
                <Chip
                  icon={<Cancel />}
                  label={`Rejected: ${data?.meta?.rejected || 0}`}
                  color="error"
                  variant={
                    filters.status === "rejected" ? "filled" : "outlined"
                  }
