...
const FeedbackFormModal: React.FC<FeedbackFormModalProps> = ({ open, onClose, preSelectedType, feedbackId }) => {

    const theme = useTheme()
    const fullScreen = useMediaQuery(theme.breakpoints.down("md"))
    const [createFeedback] = useCreateFeedbackMutation()
    const [updateFeedback] = useUpdateFeedbackMutation();
    const { data: singleFeedback, isLoading } = useGetSingleFeedbackQuery(feedbackId)
    ...
    const handleSubmit = async (values: any) => {
        try {
            ...
            if (res.success) {
                toast.success(res.message || (feedbackId ? "Feedback updated successfully!" : "Feedback added successfully!"))
            } else {
    ...
    const defaultValue = {
        attachments: singleFeedback?.data?.attachments || [],
        category: singleFeedback?.data?.category || "",
        department: singleFeedback?.data?.department || "",
        description: singleFeedback?.data?.description || "",
        priority: singleFeedback?.data?.priority || "",
        title: singleFeedback?.data?.title || "",
        type: singleFeedback?.data?.type || "",
        submittedAt: singleFeedback?.data?.submittedAt || "",
    }
    useEffect(() => {
        if (singleFeedback?.data) {
            setFormData((prev) => ({
                ...prev,
                type: singleFeedback.data.type || preSelectedType || "",
                priority: singleFeedback.data.priority || "",
            }))
        }
    }, [singleFeedback, preSelectedType])
