...
export default function ClassReportForm({ id }: ClassReportProps) {
  ...
  // API queries
  const { data: singleClassReport, isLoading: isReportLoading } = useGetSingleClassReportQuery(
    { id },
    { skip: !isEditMode },
  )

  const { data: classData, isLoading: isClassLoading } = useGetAllClassesQuery({
    limit: 100,
    page: 1,
    searchTerm: "",
  })

  const { data: subjectData, isLoading: isSubjectLoading } = useGetAllSubjectsQuery({
    limit: 100,
    page: 1,
    searchTerm: "",
  })
  ...
  const [updateClassReport] = useUpdateClassReportMutation()
  ...
  const storedUser = getFromLocalStorage("user-info") ? JSON.parse(getFromLocalStorage("user-info") || "{}") : {}
  ...
  // Extract default values from the single class report
  ...
  useEffect(() => {
    if (isEditMode && singleClassReport?.data?.studentEvaluations?.length > 0) {
  ...
  }, [students, singleClassReport, isEditMode])

  // Refetch students when class filter changes
  useEffect(() => {
    if (filters.class) {
      refetchStudents()
    }
  ...
  const handleSubmit = async (data: FieldValues) => {
    ...
    let teacherValue = storedUser?.userId || storedUser.userId;
    ...
    // Ensure studentEvaluations has the correct format
    const formattedStudentEvaluations = studentEvaluations.map(evaluation => ({
      studentId: evaluation.studentId,  // Make sure this is just the ID string
      lessonEvaluation: evaluation.lessonEvaluation,
      handwriting: evaluation.handwriting,
      attendance: evaluation.attendance,
      parentSignature: evaluation.parentSignature,
    ...
    const formattedData = {
      ...
      hour: data.hour,
      date: data.date,
      studentEvaluations: formattedStudentEvaluations,
...
// In your frontend handleConfirmSubmit function
const handleConfirmSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode && id) {
      ...
      console.log('About to update with data:', JSON.stringify(formData, null, 2));
      
      // Update existing report
      response = await updateClassReport({ id, body: formData }).unwrap();
      console.log('Update API response:', response);
    } else {
      // Create new report
      response = await createClassReport(formData).unwrap();
    }
  ...
  const handleFilterChange = (filterName: string, value: string) => {
    ...
    // If class filter changes, refetch students
    if (filterName === "class") {
      refetchStudents()
    }
  }
      ...
      <ThemeProvider theme={theme}>
        <CraftForm onSubmit={handleSubmit} defaultValues={defaultValues}>
                ...
                <Box>
                  ...
                  >
                    <Typography variant="h4" component="h1" sx={{ fontWeight: 700, color: "text.primary" }}>
                      {isEditMode ? "Edit Class Report" : "+ Add New Report"}
                    </Typography>
                            ...
                            <CraftIntAutoComplete
                              ...
                              onChange={(event, newValue) => {
                                if (newValue && typeof newValue === "object") {
                                  handleFilterChange("class", newValue.value)
                                }
                              }}
                            />
                          </Grid>
                                  ...
                                  <TableCell colSpan={6} align="center" sx={{ py: 8 }}>
                                    <Box sx={{ textAlign: "center" }}>
                                      ...
                                      <Typography variant="body2" color="text.secondary">
                                        Try selecting a class to find students.
                                      </Typography>
                                    </Box>
                                  </TableCell>
                                </TableRow>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                Are you sure you want to {isEditMode ? "update" : "submit"} this class report? Please verify all student
