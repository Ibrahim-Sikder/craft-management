...
export default function SectionForm({id}:any) {
  ...
  const handleSubmit = async (data: FieldValues) => {
    ...
    try {
      ...
      setSubmittingForm(false) // Reset form submission state
    } catch (err: any) {
  ...
  const handleNext = async () => {
    // Get all form fields on the current step
    const currentStepFields = document.querySelectorAll(`[data-step="${activeStep}"] input, [data-step="${activeStep}"] textarea, [data-step="${activeStep}"] select`)
    ...
    if (isValid) {
      if (activeStep === steps.length - 1) {
        triggerFormSubmit() // Submit the form on last step
      } else {
  ...
  const formatClassForAutocomplete = (classData: any) => {
    ...
    const matchingClass = classOption.find((c: { label: string; value: string }) => c.value === classId);
  ...
  const formatTeacherForAutocomplete = (teacherData: any) => {
    ...
    const matchingTeacher = teacherOption.find((t: { label: string; value: string }) => t.value === teacherId);
  ...
  const formatRoomForAutocomplete = (roomData: any) => {
    ...
    const matchingRoom = roomOption.find((r: { label: string; value: string; capacity?: number }) => r.value === roomId);
                    ...
                    >
                      <Box sx={{ p: 3 }}>
                        ...
                        >
                          ...
                          {/* Hidden submit button - this will be triggered programmatically */}
                          <input
                            type="submit"
                            style={{ display: 'none' }}
