...
const GradingPage = () => {
  ...
  const handleGradeInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setGradeFormData({
      ...gradeFormData,
      [name]:
        name === "minMarks" || name === "maxMarks" || name === "gpa"
          ? Number(value)
          : value,
    });
  };
  ...
  const handleSubmitGrade = () => {
    ...
    setTimeout(() => {
      ...
      if (editGrade) {
        ...
        const updatedSystems = gradingSystems.map((system, index) =>
          index === selectedTab
            ? { ...system, grades: [...system.grades, newGrade] }
            : system
        );
  ...
  const handleDeleteGrade = (id: number) => {
    const currentSystem = gradingSystems[selectedTab];
    const updatedGrades = currentSystem.grades.filter(
      (grade) => grade.id !== id
    );
    ...
    <Box sx={{ p: 3 }}>
            ...
            >
              ...
              <Box sx={{ display: "flex", gap: 2 }}>
                <Button
                  ...
                  size="small"
                  onClick={() =>
                    handleOpenSystemDialog(gradingSystems[selectedTab])
                  }
                >
                  Edit System
                </Button>
      ...
      >
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <FormControlLabel
                control={
                  <Switch
                    checked={systemFormData.isActive}
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button
            onClick={handleCloseSystemDialog}
            variant="outlined"
            color="inherit"
          >
            Cancel
