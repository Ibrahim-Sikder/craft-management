...
export default function DailyClassReportForm({ id }: { id?: string }) {
  ...
  const { data: singleDailyReport, isLoading: isLoadingReport } =
    useGetSingleDailyClassReportQuery({ id }, { skip: !id });
  ...
  useEffect(() => {
    if (id && singleDailyReport && singleDailyReport.success && !dataLoaded) {
      ...
      if (reportData.classes && reportData.classes.length > 0) {
        const newRows = reportData.classes.map(
    ...
    const simulateProgress = () => {
      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          const apiCall = id
            ? updateDailyClassReport({ id, body: reportData })
            : createDailyClassReport(reportData);
          ...
          apiCall
            ...
            .then(() => {
              ...
              setTimeout(() => {
                router.push("/dashboard/teacher/daily-report/list");
              }, 1500);
            })
            .catch((error) => {
    ...
    >
      ...
      >
        ...
        >
          ...
          <MuiLink
            component={Link}
            href="/dashboard/teacher/daily-report/list"
            underline="hover"
            color="inherit"
            sx={{ display: "flex", alignItems: "center" }}
          >
            ...
            <CardContent sx={{ p: { xs: 2, sm: 4 } }}>
              <form onSubmit={handleSubmit}>
                  ...
                  <Grid container spacing={3} sx={{ mt: 1 }}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      >
                        <Typography
                          variant="subtitle1"
                          sx={{ mr: 2, fontWeight: 600, color: "#475569" }}
                        >
                          তারিখ:
                        </Typography>
                        <TextField
                ...
                >
                  <Table
                    sx={{ minWidth: 650 }}
                    aria-label="daily class report table"
                  >
                    <TableHead>
                            ...
                            <TableCell>
                              <FormControl fullWidth size="small">
                                ...
                                >
                                  ...
                                  {subjectData?.data?.subjects.map(
                                    (sub: any, i: number) => (
                                      <MenuItem key={i} value={sub.name}>
                                        {sub.name}
                                      </MenuItem>
                                    ),
                                  )}
                              ...
                              <FormControl fullWidth size="small">
                                ...
                                >
                                  ...
                                  {classData?.data?.classes?.map(
                                    (sub: any, i: number) => (
                                      <MenuItem key={i} value={sub.className}>
                                        {sub.className}
                                      </MenuItem>
                                    ),
                                  )}
                                    ...
                                    <Chip
                                      ...
                                      color={
                                        calculateLearningPercentage(row) > 70
                                          ? "success"
                                          : calculateLearningPercentage(row) >
                                              40
                                            ? "warning"
                                            : "error"
                                      }
                ...
                >
                  <Box sx={{ textAlign: "center" }}>
                    ...
                    <Typography
                      variant="subtitle1"
                      sx={{ mt: 1, fontWeight: 500, color: "#475569" }}
                    >
                      সমন্বয়কারী
                    </Typography>
                  </Box>
