...
//   // Generate standard items for a given category name based on current toggles
//   const getStandardItemsForCategory = useCallback(
//     (catName: string) => {
//       const items: any[] = [];
//       feeCategories
//         .filter(
...
//   // ── KEY FIX: Apply toggles by filtering existing items, NOT rebuilding from scratch ──
//   // This prevents feeType values from being reset when toggles change.
//   const applyFiltersToAllGroups = useCallback(() => {
...
//       // Simply filter out items that match the hidden toggles.
//       // Do NOT rebuild from scratch — preserve all user edits.
...
//         if (hideAdmissionFee && feeTypeStr === "Admission Fee") return false;
//         if (hideMonthlyFee && feeTypeStr.toLowerCase().includes("monthly"))
//           return false;
//         return true;
//       });
...
//     });
//   }, [getValues, setValue, hideAdmissionFee, hideMonthlyFee]);
...
//   const totalFees = (watch("fees") || []).reduce(
//     (t: number, f: any) =>
//       t +
//       (f.feeItems || []).reduce(
...
//   const totalDiscount = (watch("fees") || []).reduce(
//     (t: number, f: any) =>
//       t +
//       (f.feeItems || []).reduce((s: number, i: any) => {
...
//         if (
//           i.isMonthly &&
//           i.discountRangeStart &&
//           i.discountRangeEnd &&
//           i.discountRangeAmount
//         ) {
//           const si = MONTHS.indexOf(i.discountRangeStart),
//             ei = MONTHS.indexOf(i.discountRangeEnd);
//           if (si !== -1 && ei !== -1 && si <= ei)
//             rd = (parseFloat(i.discountRangeAmount) || 0) * (ei - si + 1);
//         }
//         return s + d + rd;
//       }, 0),
//     0,
...
//   const totalAdvance = (watch("fees") || []).reduce(
//     (t: number, f: any) =>
//       t +
//       (f.feeItems || []).reduce(
//         (s: number, i: any) => s + (parseFloat(i.advanceAmount) || 0),
//         0,
//       ),
//     0,
//   );
...
//       const suggested = netPayable - totalAdvance;
//       setPaidAmount(suggested > 0 ? suggested : 0);
//     }
//   }, [netPayable, totalAdvance]);
...
//                   val: `-৳${totalDiscount.toLocaleString()}`,
//                   color: "warning.main",
//                   bg: alpha(theme.palette.warning.light, 0.1),
//                 },
//                 {
//                   label: "Total Payable",
//                   val: `৳${netPayable.toLocaleString()}`,
//                   color: "primary.main",
//                   bg: alpha(theme.palette.primary.light, 0.1),
//                 },
//                 {
//                   label: "Advance Used",
//                   val: `৳${totalAdvance.toLocaleString()}`,
//                   color: "info.main",
//                   bg: alpha(theme.palette.info.light, 0.1),
//                 },
//               ].map((item) => (
