...
export default function MealReportForm({ params }: MealReportId) {
  const { id } = params
...
const router = useRouter()
  ...
  // Initialize selected persons with meals from singleMealReport
  useEffect(() => {
    if (persons.length > 0 && singleMealReport?.data) {
      const initialSelection: Record<string, boolean> = {}
      const initialMeals: Record<string, string[]> = {}

      // Initialize all as not selected
      persons.forEach((person) => {
        initialSelection[person.id] = false
        initialMeals[person.id] = []
      })

      // Select students from the meal report
      singleMealReport.data.students.forEach((student: any) => {
        initialSelection[student.personId] = true
        // Convert API meal types to UI-friendly format
        initialMeals[student.personId] = student.mealTypes.map(
          (type: string) => reverseMealTypeMapping[type as keyof typeof reverseMealTypeMapping],
        )
      })

      // Select teachers from the meal report
      singleMealReport.data.teachers.forEach((teacher: any) => {
        initialSelection[teacher.personId] = true
        // Convert API meal types to UI-friendly format
        initialMeals[teacher.personId] = teacher.mealTypes.map(
          (type: string) => reverseMealTypeMapping[type as keyof typeof reverseMealTypeMapping],
        )
      })
      ...
      const allSelected = persons.every((person) => initialSelection[person.id])
      setSelectAll(allSelected)
    }
  }, [persons, singleMealReport])
  ...
  // Set date from singleMealReport
  useEffect(() => {
    if (singleMealReport?.data?.date) {
      setDate(new Date(singleMealReport.data.date))
    }
  }, [singleMealReport])
