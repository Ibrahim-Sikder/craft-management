...
export default function TodayTask({
  ...
  const { data: singleTodayTask, isLoading } = useGetSingleTodayTaskQuery(
    id || "",
    {
      skip: !id,
    },
  ...
  const defaultValues = {
    dueDate: singleTodayTask?.data?.dueDate || getTodayDate(),
  ...
  const handleSubmit = async (data: FieldValues) => {
    try {
      ...
      if (!id) {
        ...
        const res = await updateTodayTask({ id, data: submitData }).unwrap();
        if (res.success) {
          toast.success("বাড়ির কাজের বিষয়বস্তু কমপক্ষে ৫ অক্ষরের হতে হবে।");
          onClose();
          onSave(res.data._id);
        }
      }
      ...
      toast.error(
        error?.data?.message || "আজকের বাড়ির কাজ সংরক্ষণ করতে ব্যর্থ হয়েছে",
      );
    } finally {
      setLoading(false);
    }
